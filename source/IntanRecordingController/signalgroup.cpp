...
QDataStream& operator>>(QDataStream &inStream, SignalGroup &a)
{
    ...
    // New in v2.06:
    // Hack to reconstruct the commandStream parameter, since we forgot to save it in the SignalChannel << method.
    // Will not work properly if a port has a chip connected to MISO2 and not MISO1, but this should be rare.
    int portIndex = 0;
    if (a.prefix == QString("A")) portIndex = 4 * 0;
    if (a.prefix == QString("B")) portIndex = 4 * 1;
    if (a.prefix == QString("C")) portIndex = 4 * 2;
    if (a.prefix == QString("D")) portIndex = 4 * 3;
    if (a.prefix == QString("E")) portIndex = 4 * 4;
    if (a.prefix == QString("F")) portIndex = 4 * 5;
    if (a.prefix == QString("G")) portIndex = 4 * 6;
    if (a.prefix == QString("H")) portIndex = 4 * 7;

    for (int i = 0; i < nTotal; ++i) {
        a.addAmplifierChannel();
        inStream >> a.channel[i];
        // Hack to reconstruct the commandStream parameter, since we forgot to save it in the SignalChannel << method.
        a.channel[i].commandStream = portIndex + (a.channel[i].boardStream - a.channel[0].boardStream);
    }
